...
// Fungsi untuk mendapatkan Volume Serial Number dari drive saat ini
...
// Fungsi untuk convert DWORD ke string
...
int WEBCHECK(const wchar_t* url) {
    ...
    while (std::getline(stream, line)) {
        ...
        // PERBAIKAN KEAMANAN: Dapatkan HWID lokal untuk validasi
        DWORD localHWID = GetVolumeSerialNumberFromCurrentDrive();
        std::string localHWIDStr = ConvertToString(localHWID);

        // CRITICAL FIX: Validasi HWID harus match SEBELUM cek tanggal
        if (serverHWID != localHWIDStr) {
            continue; // HWID tidak match, skip ke line berikutnya
        }

        // Setelah HWID match, baru validasi tanggal
