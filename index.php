...
/**
 * (1) & (2) Tetapkan parameter cookie sesi sebelum session_start()
 * - secure + httponly + SameSite=Lax agar flow pembayaran & redirect tetap mulus
 */
if (PHP_VERSION_ID >= 70300) {
    ...
    session_set_cookie_params(0, '/; Secure; HttpOnly; SameSite=Lax');
}
...
// --- PENYESUAIAN: HANYA UNTUK PRODUKSI ---
// Langsung atur URL dan Kunci untuk Midtrans Produksi.
$midtrans_production_url = 'https://app.midtrans.com';
$midtrans_production_client_key = $_ENV['MIDTRANS_CLIENT_KEY_PROD'] ?? ''; // Ambil Client Key Produksi dari .env

/**
 * (6) Header keamanan tambahan
 * (ditempatkan sebelum CSP)
 */
...
/**
 * (5) Atur Content Security Policy (CSP) lebih ketat namun tetap kompatibel Snap Midtrans
 * - Tambah *.midtrans.com & *.veritrans.co.id
 * - object-src none, base-uri self, frame-ancestors self, upgrade-insecure-requests
 */
$csp = "default-src 'self'; ";
$csp .= "script-src 'self' {$midtrans_production_url} https://*.midtrans.com https://*.veritrans.co.id https://cdn.jsdelivr.net 'unsafe-inline'; ";
...
$csp .= "connect-src 'self' {$midtrans_production_url} https://api.midtrans.com https://*.midtrans.com https://*.veritrans.co.id; ";
$csp .= "img-src 'self' data: {$midtrans_production_url} https://*.midtrans.com https://*.veritrans.co.id; ";
$csp .= "frame-src {$midtrans_production_url} https://*.midtrans.com https://*.veritrans.co.id; ";
$csp .= "object-src 'none'; ";
$csp .= "base-uri 'self'; ";
...
// Terapkan metode Double Submit Cookie untuk CSRF
$csrf_token_value = '';
if (empty($_SESSION['csrf_token'])) {
    ...
    /**
     * (1) Cookie CSRF secure/httponly/SameSite=Lax agar tidak bocor & tidak putus setelah redirect
     */
...
function get_visitor_ip() {
    ...
    } elseif (!empty($_SERVER['HTTP_CF_CONNECTING_IP'])) { // Khusus untuk Cloudflare
...
try {
    $stmt_log = $conn->prepare("INSERT INTO visitor_logs (ip_address, user_agent) VALUES (?, ?)");
    // 'ss' berarti kita mengirim dua parameter string
    $stmt_log->bind_param('ss', $visitor_ip, $user_agent);
    $stmt_log->execute();
    $stmt_log->close();
} catch (Exception $e) {
    // Jika ada error (misal tabel belum dibuat), jangan hentikan script.
    // Anda bisa mencatat error ini di file log terpisah jika perlu.
    error_log('Gagal menyimpan log pengunjung: ' . $e->getMessage());
}
...
<head>
    ...
    <script type="text/javascript" src="<?php echo htmlspecialchars($midtrans_production_url); ?>/snap/snap.js" data-client-key="<?php echo htmlspecialchars($midtrans_production_client_key); ?>"></script>
    
    <link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/sweetalert2@11/dist/sweetalert2.min.css">
    <link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/bootstrap-icons@1.11.3/font/bootstrap-icons.min.css">
    <style>
        /* Gaya CSS Anda (tidak diubah) */
        ...
        <div class="product-selection">
            <div class="selection-grid product-grid">
                <?php foreach ($products as $p): ?>
                    <?php
                        // (3) & (4) Validasi/casting angka + escape output sebelum dicetak
                        $data_id     = (int)($p['id'] ?? 0);
                        $data_harga  = (int)($p['harga'] ?? 0);
                        $durasi_hari = isset($p['durasi_hari']) ? (int)$p['durasi_hari'] : 0;
                        $data_durasi = htmlspecialchars($durasi_hari . ' Hari', ENT_QUOTES, 'UTF-8');
                        $nama_produk = htmlspecialchars($p['nama_produk'] ?? '', ENT_QUOTES, 'UTF-8');
                        $deskripsi   = nl2br(htmlspecialchars($p['deskripsi'] ?? 'Deskripsi tidak tersedia.', ENT_QUOTES, 'UTF-8'));
                    ?>
                    <div class='selection-card' data-product-id='<?php echo $data_id; ?>' data-harga='<?php echo $data_harga; ?>' data-durasi='<?php echo $data_durasi; ?>'>
                        <div><i class='bi bi-patch-check'></i>
                        <h5><?php echo $nama_produk; ?></h5>
                        <p class='description'><?php echo $deskripsi; ?></p></div>
                        <div><p>Rp <?php echo number_format($data_harga); ?></p></div>
                    </div>
                <?php endforeach; ?>
            </div>
        </div>
