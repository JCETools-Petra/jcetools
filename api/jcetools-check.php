...
// [PERBAIKAN] Mengubah path untuk memuat vendor dari direktori root
require_once __DIR__ . '/../vendor/autoload.php';

try {
    // [PERBAIKAN] Mengubah path untuk memuat file .env dari direktori root
    // __DIR__ -> /path/to/your/project/api
    // __DIR__ . '/..' -> /path/to/your/project
    $dotenv = Dotenv\Dotenv::createImmutable(__DIR__ . '/..');
    $dotenv->load();

} catch (Exception $e) {
    // Jika .env tidak ditemukan atau ada error lain
...
// Query ke Database
try {
    // Perbaikan: Ambil juga kolom 'Nama' untuk respons
    ...
    if ($result->num_rows > 0) {
        ...
        if (time() > $expiry_date) {
            ...
            // Perbaikan: Tambahkan log ke success.log
            log_message('success', 'HWID valid untuk user ' . $row['Nama'] . '. Kedaluwarsa: ' . $row['expiry_date']);
            ...
            // Perbaikan: Tambahkan nama pengguna ke respons
            $response = ["status" => "success", "message" => "HWID valid", "expiry_date" => $row["expiry_date"], "nama_pengguna" => $row["Nama"]];
        }
    } else {
